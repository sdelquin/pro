...
Esta sección explica cómo montar **la máquina virtual** con la que vamos a trabajar en el módulo de PRO.
...
```console
curl http://amy/daw/1daw/pro/build.sh | bash -s pro
```
...
```console
curl http://amy/daw/1daw/pro/fixnet.sh | bash -s pro
```
...
2. Abre una terminal y lanza el siguiente comando para comprobar que tu IP se corresponde con la que debe. Ejemplo: Si tu número de PC es el 12, el comando debería dar como salida: `10.103.12.20`
